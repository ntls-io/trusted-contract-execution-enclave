...
#
# The enclave build stages optionally take an existing signing key as input.
# To pass this to the enclave build steps, use something like:
#
#   --secret id=sgx-signing-key,src=<path>/Enclave_private.pem
#
# (If an existing key is not provided, the build will generate a new key.)
#
# XXX: Note that changing the key does NOT invalidate the docker instruction cache:
#      the key will only be used for uncached steps.
#
...
# Intel SGX SDK 2.20
# Docs:
ARG INTEL_SGX_SDK_INSTALLER_URL="https://download.01.org/intel-sgx/sgx-linux/2.20/distro/ubuntu20.04-server/sgx_linux_x64_sdk_2.20.100.4.bin"

# Rust SGX SDK 1.1.6
ARG RUST_SGX_SDK_REVISION="3c903bdac4e503dd27b9b1f761c4abfc55f2464c"
...
# Builder for Rust, with Rust toolchain and cargo build tools (bindgen, cargo make) under /home/user/.cargo
...
WORKDIR /build/trusted-contract-execution-enclave
COPY --chown=user . .

# STAGE: build-sgx-wallet-server (from web-server-builder, arg SGX_MODE, secret sgx-signing-key)
# Build the sgx-wallet server, in the given mode.
FROM web-server-builder AS build-execution-enclave-server

ARG SGX_MODE
RUN --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/home/user/.cargo/registry \
    --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/home/user/.cargo/git \
    --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/build/trusted-contract-execution-enclave/app/target \
    --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/build/trusted-contract-execution-enclave/enclave/target \
    --mount=type=secret,uid=1000,gid=1000,id=sgx-signing-key,target=/build/trusted-contract-execution-enclave/keys/Enclave_private.pem \
    . ~/.cargo/env && \
...
# STAGE: build-cargo-make-ci (from web-server-builder, arg SGX_MODE)
# Build project with cargo make
FROM web-server-builder AS build-cargo-make-ci

#
COPY --from=build-execution-enclave-server /build/trusted-contract-execution-enclave/build/lib/ /build/trusted-contract-execution-enclave/build/lib/

ARG SGX_MODE
RUN --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/home/user/.cargo/registry \
    --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/home/user/.cargo/git \
    --mount=type=cache,sharing=locked,uid=1000,gid=1000,target=/build/trusted-contract-execution-enclave/target \
    . ~/.cargo/env && \
    . /build/intel/sgxsdk/environment && \
#    CUSTOM_COMMON_PATH="/build/rust-sgx-sdk/common" \
#    CUSTOM_EDL_PATH="/build/rust-sgx-sdk/edl" \
...
FROM base-sgx-runner-sw AS run-trusted-contract-execution-enclave-server-sw

WORKDIR /app
COPY --from=build-execution-enclave-server /build/trusted-contract-execution-enclave/bin/enclave.signed.so /app
COPY --from=build-execution-enclave-server /build/trusted-contract-execution-enclave/bin/app /app
...
FROM base-sgx-runner-hw AS run-trusted-contract-execution-enclave-server-hw
